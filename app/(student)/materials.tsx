...
export default function StudentMaterialsScreen() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    listStudyMaterialsForClass(user, user.classId)
      .then((data) => {
        setMaterials(data);
        setFiltered(data);
      })
  ...
  const handleDownload = (material: StudyMaterial) => {
    Alert.alert(
      'Download Started',
      `Downloading "${material.title}" (${material.fileType.toUpperCase()}) from security server...`,
      [{ text: 'OK' }]
    );
  };
  ...
  return (
    <Screen title="Study Materials" subtitle="Notes, presentations, and guides for your classes" loading={loading} scroll>
