...
export default function ParentHomeworkScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getHomeworkForStudent(actor, activeChild.id);
        setRows(data);
      } catch (e) {
  ...
  return (
    <Screen title="Homework Assignments" subtitle={`Homework for ${activeChild?.name}`} loading={loading} scroll embedded>
